---
name: ghostbill-frontend
description: This skill should be used when the user asks to implement or extend the Ghostbill frontend only, build the React UI for Ghostbill, implement the upload interface and results display, work on App.tsx / App.css / hooks, or run the frontend-only Ghostbill v2 implementation prompt.
version: 2.0.0
argument-hint: [optional notes or scope override]
allowed-tools: [Read, Write, Edit, Glob, Grep, Bash]
---

# Ghostbill Frontend Implementation — v2
...
Ghostbill is an expense-awareness tool — not a budgeting app or income tracker. Users upload a bank file. The backend analyzes repeated outgoing charges and classifies them as:
...
| `annualCost` | `Math.round(averageAmount × annualMultiplier(cadenceDays))` | Annual projection on ghost rows |
...
Signature: `useAnalysisMemo(result: AnalysisResult | null, prevGhostNames: Set<string>)`
...
- **File preview** (shown immediately after selection, before upload):
  ...
  - Estimated row count for CSV and JSON only (best-effort)
...
2. **"New this scan" badge** — ghost rows only, shown when `isNewThisUpload === true`. Style: green tint (`background: rgba(30,95,87,0.09)`, `color: #1a5e56`).

3. **"New · may be a trial" badge** — ghost rows only, shown when `isNewGhost === true` AND `isNewThisUpload === false`. Style: red tint.
...
8. **"I know about this" dismiss button** — ghost rows only. Small text button, no border.
...
Right side of the ghost section header:

1. **"Download list" button** — downloads a CSV of active (non-dismissed) ghost merchants with columns: `Merchant,Per charge (SEK),Times charged,Already paid (SEK),Est. annual (SEK)`. Disabled when all ghosts are dismissed. Client-side Blob download — no backend call.
...
- **Content:** 8 most recent transactions from all repeated groups (ghosts + regulars combined), sorted by `transaction.date` descending
- **Each card:** merchant name · amount in SEK (absolute value) · date as "MMM D"
- **Layout:** horizontal scrollable card grid
- Do not render when there are no results
...
- [ ] Clicking dismiss moves merchant to "Known charges" section
