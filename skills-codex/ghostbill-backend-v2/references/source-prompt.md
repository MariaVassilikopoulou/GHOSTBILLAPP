...
```
RowTolerance      = 5.0   // Y-coordinate grouping tolerance (see note below)
...
forms with independent coordinate systems. Words on the same visual row but in
different form elements can have Y positions differing by up to ~4 pts — causing
RowTolerance=2.5 to split them into separate buckets, both of which get dropped.

Amount fallback: after JoinWords(AmountLeft, AmountRight), if amount is still empty,
scan ALL words on the row (no X constraint) matching SwedishAmountRegex = ^-?[\d ]+,\d{2}$.
Prefer the first negative token (expense); if none, take the first matching token.
Negative-first avoids accidentally picking the running balance (always positive).

Description fallback: after JoinWords(DescriptionLeft, DescriptionRight), if description
is still empty, take all words that are neither a date (^\d{4}-\d{2}-\d{2}$) nor a
SwedishAmountRegex match, joined with spaces.
